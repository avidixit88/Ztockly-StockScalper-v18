...
def load_email_secrets():
    """Load email secrets from either [email] table or flat keys."""
...
cooldown_minutes = st.sidebar.slider("Cooldown minutes (per ticker)", 1, 60, 10, 1)
alert_threshold = st.sidebar.slider("Alert score threshold", 60, 100, int(PRESETS[mode]["min_actionable_score"]), 1)

capture_alerts = st.sidebar.toggle(
    "Capture alerts in-app",
    value=True,
    help="Stores alerts inside the app. Note: Streamlit must be open to run scans/alerts (no background execution).",
)
...
smtp_server, smtp_port, smtp_user, smtp_app_password, to_address = load_email_secrets()

if enable_email_alerts:
    if not (smtp_user and smtp_app_password and to_address):
        st.sidebar.warning("Email is ON but Secrets are missing. Add SMTP_USER / SMTP_APP_PASSWORD / ALERT_TO in Streamlit Secrets.")
    else:
        st.sidebar.success(f"Email enabled → {to_address}")
...
with tab_scan:
    ...
    with col_b:
        ...
        if st.button("Capture test alert"):
            # Deterministic test payload so you can validate both in-app + email wiring (no market dependency).
            test = {
                "Symbol": "TEST",
                "Bias": "LONG",
                "Score": 95,
                ...
                "ATR%": 0.008,
                "ATR baseline%": 0.008,
                "Scale note": "Test payload — no normalization.",
            }

            # Always capture in-app so you can see it render even if scanning is off.
            add_in_app_alert(test)
            st.success("Test alert captured in-app.")

            # Optionally send email too (if configured in Secrets + toggle enabled)
            if enable_email_alerts:
                smtp_server, smtp_port, smtp_user, smtp_app_password, to_address = load_email_secrets()

                if smtp_user and smtp_app_password and to_address:
                    subject = "Ztockly Test Alert"
            body = format_alert_email(test)
                    try:
                    send_email_alert(
                        smtp_user=smtp_user,
                    ...
                    if ok:
                        st.success(f"Test email sent to {to_address}.")
                    else:
                        st.error(f"Test email failed: {err}")
                else:
                    st.warning("Email toggle is ON, but SMTP secrets are missing. Add SMTP_USER / SMTP_APP_PASSWORD / ALERT_TO in Streamlit Secrets.")
    with col_c:
...
# Alert firing (in-app and/or email)
now = time.time()
for r in results:
    if r.bias in ["LONG", "SHORT"] and r.setup_score >= alert_threshold:
        if can_alert(r.symbol, now, cooldown_minutes):
            ...
            if enable_email_alerts:
                subject = f"Ztockly Alert: {row.get('Symbol', '?')} {row.get('Bias', '')}"
                        body = format_alert_email(row)
                try:
                    send_email_alert(
